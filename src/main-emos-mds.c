...
void print_each_iset(){ //affiche le contenu de tous les IS non vide

  for(int j=0;j<MAXIS;j++){
    if(USED(iSET[j])){ // si n'est pas vide
    printf("iSET %d={",j); 
    for_each_vec_item(iSET[j],int,it){ //affiche les membre de l'is
      printf("%d ",*it);
    }
    printf("}\n");
    }
...
static int print_final_solution(char *inst){
   //printf("--------------------------------\n");
   //printf("Solution: ");
...
static void initialize(){
  ...
  //initialize stack
  new_stack(BRA_STK,VEC_INT,int,NB_NODE); // new_stack(NOM DE LA PILE, TYPE DE VECTEUR, TYPE, TAILLE)
...
int final(int argc, char *string){
  ...
  if(read_instance(argv[1])) {
    ...
    f = print_final_solution(getInstanceName(argv[1]));
